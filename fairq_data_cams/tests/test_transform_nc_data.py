...
def test_df_from_nc_file() -> None:
    res = df_from_nc_file(file_path=Path(__file__).parent / "data" / "ENS_FORECAST_2022-05-24.nc")
...
def test_add_date_columns() -> None:
    input_df = pd.DataFrame({"time": [timedelta(hours=x) for x in [0, 1, 24, 95]]})
    res = add_date_columns(file_path=Path(__file__).parent / "data" / "ENS_FORECAST_2022-05-24.nc", df=input_df)
